...
<head>
    ...
    <title>Blog webite</title>
</head>
<body>
    <!--blog-edit update code -->
        ...
        <?php foreach ($query_run as $row) { ?>
            <form method="POST">
                ...
                <input type="text" placeholder="Blog Title" class="form-control my-3 bg-dark text-white text-center"
                    name="tittle" value="<?php echo $row['tittle'] ?>">
                <textarea name="content" class="form-control my-3 bg-dark text-white" cols="30"
                    rows="10"><?php echo $row['content'] ?></textarea>


                <input type="submit" id="button" class="btn btn-dark" name="update">Update</button>
            </form>
        <?php } ?>
    </div>
